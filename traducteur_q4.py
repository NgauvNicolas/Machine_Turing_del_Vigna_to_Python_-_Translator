...
Exemple d'appel en ligne de commande sur le terminal : python traducteur_q4.py ts_scripts/multiplicateur.1.TS 3 5
    Et on obtient le Script Python généré : py_scripts/multiplicateur.1_Q4translated.py
On peut ensuite tester le Script Python qui a été créé à partir de la traduction du Script TS :
    Exemple d'appel en ligne de commande sur le terminal : python py_scripts/multiplicateur.1_Q4translated.py 3 5
...
class MTdV_Traducteur_q4 :
    ...
    def generer_corps(self, nom_fichier) :
        ...
        if len(sys.argv) == 2 :
            ...
            self.ajouter_ligne("ruban = init_ruban([int(sys.argv[1])+1])")
            self.ajouter_ligne("X = [len_v2(ruban) // 2]")
        elif len(sys.argv) == 4 :
            ...
            while i < len(tokens) :
                if tokens[i] == "D" :
                    ...
                    if self.if_compteur > 0 :
                        ...
                        #self.ajouter_ligne("return boucle{}(ruban, X)".format(self.boucle_nom_pos_ind[-1][0]))
                        self.else_compteur -= 1
                        self.indentation_courante -= 1
                        self.ajouter_ligne("")
...
if __name__ == "__main__" :
    traducteur_q4 = MTdV_Traducteur_q4()
    translated_code = traducteur_q4.traduire_fichier(sys.argv[1])

    # Afficher le code traduit
    #print(translated_code)

    # Sauvegarder dans un fichier de sortie
    output_fichier = sys.argv[1].replace("ts_scripts", "py_scripts") .replace(".TS", "_Q4translated.py")
